...
$con = connect();
$id = $_GET['id'];
$sql = "SELECT medico.coddoc, medico.dnidoc, medico.nomdoc, medico.apedoc, especialidades.codespe, especialidades.especialidad ,medico.sexo, medico.telefo, medico.ciudad, medico.fechanaci, medico.correo, medico.estado, medico.fecha_create FROM medico INNER JOIN especialidades ON medico.codespe = especialidades.codespe  WHERE coddoc= '$id'";
$query  =$con->query($sql);
$data =  array();
if($query){
...
<?php if(count($data)>0):?>
          ...
          <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
            <form method="POST"  autocomplete="off" action="../../folder/doctor.php?id=<?php echo $d->coddoc; ?>"  role="form">
             ...
             <label class="block text-sm">
                ...
                <input name="dnidoc" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" maxlength="10" value="<?php echo $d->dnidoc; ?>" type="text"
              ...
              <label class="block mt-4 text-sm">
                ...
                <select  name="codespe"  class="block w-full mt-1 text-sm dark:text-gray-300 dark:border-gray-600 dark:bg-gray-700 form-select focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:focus:shadow-outline-gray">
                 <option value="<?php echo $d->codespe; ?>"><?php echo $d->especialidad; ?></option>
 ...
 $stmt = $dbcon->prepare('SELECT * FROM especialidades');
        $stmt->execute();
        ...
        while($row=$stmt->fetch(PDO::FETCH_ASSOC))
        {
            extract($row);
            ?>
            <option value="<?php echo $codespe; ?>"><?php echo $especialidad; ?></option>
            <?php
        }
        ?>
            ?>
              ...
              <label class="block text-sm">
                ...
                <input name="telefo" maxlength="10" onKeypress="if (event.keyCode < 45 || event.keyCode > 57) event.returnValue = false;" value="<?php echo $d->telefo; ?>" type="text"
                  class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
              ...
              <label class="block text-sm">
                ...
                <input name="ciudad" value="<?php echo $d->ciudad; ?>" type="text"
                  class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                  placeholder=""
                />
              </label>
