...
$con = connect();
$id = $_GET['id'];
$sql = "SELECT codcit, citas.asunto, customers.codpaci, customers.dnipa, customers.nombrep, customers.apellidop,medico.coddoc, medico.dnidoc,medico.nomdoc, medico.apedoc,calendario.idhora, calendario.nomhora,especialidades.codespe, especialidades.especialidad, citas.estado,start, end, color  FROM citas  INNER JOIN customers ON citas.codpaci = customers.codpaci INNER JOIN medico ON citas.coddoc = medico.coddoc INNER JOIN especialidades ON citas.codespe = especialidades.codespe INNER JOIN calendario ON citas.idhora =  calendario.idhora  WHERE codcit= '$id'";
$query  =$con->query($sql);
$data =  array();
if($query){
